...
def main(args):
    ...
    if args.generate:
        t_gen = run([sys.executable, str(Path(__file__).parent / "data_gen.py"), "--out", str(base), "--pages", str(args.pages), "--searches", str(args.searches), "--files", str(args.files)])
    ...
    # Naive
    t_naive = run([str(ROOT / "target/release/naive-join-text"), "--input1", str(inp_search), "--input2", str(inp_pages), "--output", str(out_naive)])

    # Single node
    env_single = os.environ.copy()
    if 'SLURM_JOB_ID' in env_single:
        env_single.pop('SLURM_JOB_ID')
    t_single = run([str(ROOT / "target/release/bjorn-join-text"), "--input1", str(inp_search), "--input2", str(inp_pages), "--output", str(out_single)], env=env_single)
    ...
    if not args.skip_multi:
        ...
        # Use workspace target path for the binary
        env["BIN_JOIN"] = str(ROOT / "target/release/bjorn-join-text")
        t_multi = run(["sbatch", "--wait", str(Path(__file__).parent / "slurm_submit_join.sbatch")], env=env)
